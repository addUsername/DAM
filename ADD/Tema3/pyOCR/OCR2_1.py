...
def doMagicRaw(imgQ, imgA):
    ...
    '''
    legacy
    text = pytesseract.image_to_string(Image.open(imgQ), config="--psm 11 --oem 3")
    textAnswer = pytesseract.image_to_string(Image.open(imgA), config="--psm 11 --oem 3")
    ...
    textAnswer = pytesseract.image_to_string(Image.open(imgA),config="-c tessedit_char_whitelist=#.·=!0123456789ABCDEFGHIJKLMNNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz")
    '''
...
def cleanOutput(questionsList, answersList):
    ...
    for i in range(3):
        ...
        if (index > len(questionsList)):
            ...
            index += 2
        questions[i] = cleanBadChar(questions[i])
    ...
    for i in range(3):
        ...
        if (index > len(answersList)):
            ...
            index += 2
        answers[i] = cleanBadChar(answers[i])

    return (questions, answers)

# Fuck unicode or whichever invalid token is in there.
# TODO Check if str.encode().decode() 
def cleanBadChar(string):
    return re.sub(u'[^\x20-\x7f]+',u' ',string)
...
def main():
    ...
    #xml.append(ET.Comment("Generated by OCR2.py for add.dam2.p3,MainModel.java"))
    # Generate String
    encoding = 'utf-8'
    raw_string = ET.tostring(root, encoding=encoding, method='xml')
    string = "<?xml version='1.0' encoding='utf-8'?>"+str(raw_string,encoding=encoding)
